...
| 存储 | 已支持核心 | Memory、SQLite、PostgreSQL、MySQL、Redis standalone/Cluster；DataExpire/FinishedDataExpire 有界 CAS 清理覆盖五后端；关系型后端及 Redis standalone/三主三从 Cluster 往返、重定向和切换已通过 CI，网络分区证据待补 |
...
| DTM HTTP/JSON-RPC/gRPC 兼容 | 已支持核心 | 9 个 gRPC 方法、逐事务 timeout/retry/Header 及 WaitResult 异步调度；真实 Rust gRPC、固定上游 Go Saga/Message、仓库 JS/TS SDK 与 HTTP/JSON-RPC callback 错误矩阵已通过，Node/TLS/完整超时矩阵待补 |
...
| Workflow DSL | 已支持核心 | 静态依赖图、恢复和逆序补偿；callback Workflow 支持复合进度、终态及 HTTP/JSON-RPC/gRPC QueryPrepared 主动恢复，HTTP/JSON-RPC 错误码和终态竞争已通过真实服务门禁，gRPC/TLS 错误矩阵待验证 |
...
| SDK | 已支持 Web 范围 | 已提供 Rust HTTP/gRPC client，以及 TypeScript/JavaScript `/v1` 与 dtm-labs HTTP/JSON-RPC 兼容客户端；JS/TS 运行时和固定上游 Go 客户端已纳入真实服务 CI |
...
5. 在已通过的 Redis standalone/Cluster 往返、重定向和切换矩阵上补齐网络分区/fencing 故障证据、Dashboard 暗色浏览器端到端与 Roze Admin 内嵌模块。
