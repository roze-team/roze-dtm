...
`CustomedData`、`QueryPrepared` 和 `ReqExtra` 会保存在事务 metadata 中，避免跨协议转换时丢失。Saga 的 `CustomedData` 若包含上游 `{"concurrent":true,"orders":...}` 合同，还会转换为持久化分支依赖 DAG。固定上游版本的 `DtmTransOptions` 没有 Message `Concurrent` 字段，4 号位属于已废弃透传 Header，因此 gRPC 不提供非兼容扩展；并发 Message 使用 HTTP/JSON-RPC 兼容入口或 Roze 原生接口。`RetryInterval`、`RequestTimeout` 按上游合同以秒接收并转换为毫秒，`RetryLimit` 和 `BranchHeaders` 进入持久化 `TransactionOptions`：HTTP 分支调用会应用逐事务 timeout 和 Header，失败恢复使用逐事务初始退避，Saga 在重试次数耗尽后进入补偿。`WaitResult=false` 时，Submit/Abort 在完成状态校验和持久化调度后立即返回，实际分支调用由带租约的恢复 worker 推进；`WaitResult=true` 保持同步等待。Prepare 始终同步完成持久化阶段。
...
HTTP(S) callback 直接按 URL origin 校验；`grpc://`、`grpcs://` 或上游裸 `host:port/service/method` 目标分别映射到 `http://`、`https://` origin 后受同一 `allowed_branch_origins` 约束。callback Workflow 不继承服务端默认事务超时；调用方显式设置 `TimeoutToFail` 时，超时会稳定落为 `failed`。`branch_tls_ca_file` 可为私有 PKI 注入 PEM CA bundle，同时保留系统信任根并严格校验证书链和目标主机名。`examples/grpc_callback_smoke.rs` 已在真实服务中使用临时私有 CA 和带 `localhost` SAN 的服务证书，通过 `grpcs://` 验证 `FAILED_PRECONDITION`/`ABORTED`、metadata、二进制 payload、逐调用 deadline、恢复重试及全局超时终态；生产证书轮换和长期运行仍需部署环境证据。
