...
- gRPC 事务扩展字段会持久化；逐事务重试间隔、请求超时、Saga 重试上限和分支 Header 已接入核心执行器。`WaitResult=false` 的 Submit/Abort 由带租约的恢复 worker 异步推进，`WaitResult=true` 同步等待；回调式 Workflow 动态进度注册尚未完成，不能视为上游行为完全等价。
